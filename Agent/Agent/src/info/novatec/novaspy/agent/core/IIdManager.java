package info.novatec.novaspy.agent.core;
...
/**
 * The ID Manager is used to correlate between the local and global IDs (from
 * the server).
 * 
...
public interface IIdManager {
	...
	/**
	 * Starts the ID Manager so that the registration will occur after a
	...
	/**
	 * Returns if this platform is registered with the CMR, is equal to the fact
	 * that calling the {@link #getPlatformId()} method results in no
	 * {@link IdNotAvailableException}.
	 * 
	...
	/**
	 * Returns the platform id. This is unique for this java agent in this
	 * virtual machine as long as a different agent name is set on the same
	 * machine.
	 * 
	 * @return The unique platform id.
	 * @throws IdNotAvailableException
	 *             This exception is thrown if no ID can be retrieved from this
	 *             manager.
	 */
	long getPlatformId() throws IdNotAvailableException;

	/**
	 * Returns the ID of the server for this method. Needed for the data objects
	 * so they can be persisted properly.
	 * 
	 * @param methodId
	...
	/**
	 * Returns the ID of the server for this sensor type. Needed for the data
	 * objects so they can be persisted properly.
	 * 
	 * @param sensorTypeId
	 ...
	 *            The method sensor type configuration which contains all the
	 *            needed information for registering.
	 * @return The unique method sensor type id.
	 */
	long registerMethodSensorType(MethodSensorTypeConfig methodSensorTypeConfig);
	
	long registerExceptionSensorType(MethodSensorTypeConfig exceptionSensorTypeConfig);
	 ...
	 *            The platform sensor type configuration which contains all the
	 *            needed information for registering.
	 * @return The unique method sensor type id.
	 */
