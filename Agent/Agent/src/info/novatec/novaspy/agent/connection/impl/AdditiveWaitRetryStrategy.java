...
import info.novatec.novaspy.agent.connection.RetryStrategy;
 ...
 * constant amount with each retry.
 * 
 * Note that the default version of this (e.g. the one with a zero argument
 * constructor) will make 3 calls and wind up waiting approximately 11 seconds
 * (zero wait for the first call, 3 seconds for the second call, and 8 seconds
 * for the third call). These wait times are pretty small, and are usually
 * dwarfed by socket timeouts when network difficulties occur anyway.
 * 
...
public class AdditiveWaitRetryStrategy extends RetryStrategy {
	...
	/**
	 * Every time there is an exception, we add to the waiting time the
	 * specified one.
	 */
	...
	/**
	 * The default constructor which initializes the class with the predefined
	 * values.
	 */
	...
	/**
	 * This constructor takes three arguments, the first is the actual number of
	 * retries till we completely fail. The second one is the first wait time
	 * when an error occurs, and the third one is the additional wait time every
