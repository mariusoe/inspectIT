...
import info.novatec.inspectit.javassist.bytecode.ByteArray;
...
public abstract class Tracer implements TypeTag {
	...
	private void checkParamTypes(String desc, int i) throws BadBytecode {
		...
		if (array)
			TypeData.setType(stackTypes[stackTop], desc.substring(i, k), classPool);
		else if (c == 'L')
			TypeData.setType(stackTypes[stackTop], desc.substring(i + 1, k - 1).replace('/', '.'), classPool);
	}
}
