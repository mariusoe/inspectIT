 ...
 * class <tt>ClassLoader</tt> is an abstract class. Given the name of a class,
...
public abstract class ClassLoader {
	 ...
	 * SecurityManager#checkCreateClassLoader() <tt>checkCreateClassLoader</tt>}
	 ...
	 *             <tt>checkCreateClassLoader</tt> method doesn't allow
	 ...
	 * the method
	 * {@link #getSystemClassLoader() <tt>getSystemClassLoader()</tt>} as the
	 * parent class loader.
	 * 
	 * <p>
	 * If there is a security manager, its {@link
	 ...
	 * is invoked by the Java virtual machine to resolve class references.
	 * Invoking this method is equivalent to invoking {@link #loadClass(String,
	 * boolean) <tt>loadClass(name, false)</tt>}.
	 * </p>
	 * 
	 * @param name
	 ...
	 * flag is true, this method will then invoke the {@link
	 ...
	 * classes, and will be invoked by the {@link #loadClass <tt>loadClass</tt>}
	 ...
	 *            format of a valid class file as defined by the <a
	 ...
	 * java.security.ProtectionDomain) <tt>defineClass</tt>} method that
	 * takes a <tt>ProtectionDomain</tt> as one of its arguments.
	 * </p>
	 * 
	 * @param name
	 *            The expected name of the class, or <tt>null</tt> if not
	 *            known, using '<tt>.</tt>' and not '<tt>/</tt>' as the
	 ...
	 * <tt>name</tt> argument is <tt>null</tt>, this check is not
	 * performed. You should always pass in the name of the class you are
	 * defining as well as the bytes. This ensures that the class you are
	 * defining is indeed the class you think it is.
	 * 
	 * <p>
	 * The specified class name cannot begin with "<tt>java.</tt>", since
	 * all classes in the "<tt>java.*</tt> packages can only be defined by
	 * the bootstrap class loader. If the name parameter is not <tt>null</tt>,
	 * it must be equal to the name of the class specified by the byte array "<tt>b</tt>",
	 * otherwise a {@link <tt>NoClassDefFoundError</tt>} will be thrown.
	 * </p>
	 * 
	 * @param name
	 *            The expected name of the class, or <tt>null</tt> if not
	...
	protected final Class defineClass(String name, byte[] b, int off, int len,
		...
		byte[] b2 = PicoAgent.getInstance().inspectByteCode(b, name, this);
		if (null != b2) {
			len = b2.length;
			b = b2;
		}
		...
		try {
			...
			for (int i = 0; transformers != null && i < transformers.length; i++) {
				try {
					...
					byte[] tb = ((ClassFileTransformer) transformers[i])
							.transform(b, off, len);
		...
		if (protectionDomain.getCodeSource() != null) {
			java.security.cert.Certificate certs[] = protectionDomain
	...
	private synchronized void checkCerts(String name, CodeSource cs) {
		...
		java.security.cert.Certificate[] pcerts = (java.security.cert.Certificate[]) package2certs
				.get(pname);
		if (pcerts == null) {
	 ...
	 * linked as described in the "Execution" chapter of the <a
	...
	/**
	 * Returns the class with the given name if this loader has been recorded by
	 * the Java virtual machine as an initiating loader of a class with that
	 * name. Otherwise <tt>null</tt> is returned.
	 * </p>
	 * 
	 * @param name
	 *            The class name
	...
	protected native final Class findLoadedClass0(String name);
	 ...
	 *            The resource name
	 * 
	 * @return An input stream for reading the resource, or <tt>null</tt> if
	 ...
	 * will return <tt>null</tt> in such implementations if this class
	 * loader's parent is the bootstrap class loader.
	 * 
	 * <p>
	 ...
	 * <tt>ClassLoader</tt> which is used as the delegation parent. An
	 * instance is then created using this constructor with the default system
	 * class loader as the parameter. The resulting class loader is defined to
	 ...
	 * RuntimePermission#RuntimePermission(String)
	 * <tt>RuntimePermission("getClassLoader")</tt>} permission to verify
	 * access to the system class loader. If not, a <tt>SecurityException</tt>
	 * will be thrown.
	 * </p>
	 * 
	 ...
	 *             an exception is thrown by that constructor when it is
	 *             invoked. The underlying cause of the error can be retrieved
	 *             via the {@link Throwable#getCause()} method.
	 * 
	 * @revised 1.4
	 */
	 ...
	 * class loaders to define the packages for their classes. Packages must be
	 ...
	 * searches the library along the path specified as the "<tt>java.library.path</tt>"
	 * property.
	 * </p>
	 * 
	 * @param libname
	 ...
	 * 
	 * <p>
	 * Every native library reuqires a particular version of JNI. This is
	...
	static void loadLibrary(Class fromClass, String name, boolean isAbsolute) {
		...
		if (loader != null) {
			for (int i = 0; i < usr_paths.length; i++) {
				File libfile = new File(usr_paths[i], System
						.mapLibraryName(name));
				if (loadLibrary0(fromClass, libfile)) {
					return;
				}
	...
	private static boolean loadLibrary0(Class fromClass, final File file) {
		Boolean exists = (Boolean) AccessController
		...
		synchronized (libs) {
			...
			synchronized (loadedLibraryNames) {
				...
				for (int i = 0; i < n; i++) {
					NativeLibrary lib = (NativeLibrary) nativeLibraryContext
							.elementAt(i);
					if (name.equals(lib.name)) {
	 ...
	 * 
	 * <p>
	 * A subpackage of a package named p is any package whose name begins with "<tt>p.</tt>".
	 * For example, <tt>javax.swing.text</tt> is a subpackage of
	 ...
	 * effect on the actual assertion status of any class, and its return value
	 * is undefined.
	 * </p>
	 * 
	 * @param className
	...
	private void initializeJavaAssertionMaps() {
		...
		for (int i = 0; i < directives.classes.length; i++)
			classAssertionStatus.put(directives.classes[i], Boolean
					.valueOf(directives.classEnabled[i]));

		for (int i = 0; i < directives.packages.length; i++)
			packageAssertionStatus.put(directives.packages[i], Boolean
					.valueOf(directives.packageEnabled[i]));
