 ...
 * 
 */
public class DefaultTimerPlotController extends AbstractPlotController {

	/**
	 * The ID of this subview / controller.
	 */
	public static final String ID = "inspectit.subview.graph.timer";
	...
	public void update(Date from, Date to) {
		...
		if (oldTimerData.isEmpty() || to.before(oldFromDate) || from.after(dataNewestDate)) {
			...
			List<TimerData> rightData = (List<TimerData>) dataAccessService.getDataObjectsFromToDate(template, rightDate, to);
			List<TimerData> leftData = (List<TimerData>) dataAccessService.getDataObjectsFromToDate(template, from, leftDate);
			...
			List<TimerData> timerData = (List<TimerData>) dataAccessService.getDataObjectsFromToDate(template, rightDate, to);
			...
			List<TimerData> timerData = (List<TimerData>) dataAccessService.getDataObjectsFromToDate(template, from, leftDate);
