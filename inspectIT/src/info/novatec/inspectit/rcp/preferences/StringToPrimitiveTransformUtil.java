...
/**
 * Utilities to transform string or sting collections and maps to the same with primitive wraper
 * types.
 * 
 * @author Ivan Senic
 * 
		...
		if (null != parseMethod) {
			for (String toTransform : original) {
				try {
					...
					throw new PreferenceException("Error tranforming Colletion<java.lang.String> to Collection<" + elementClass.getName() + ">.", e);
				}
			}
		} else {
		...
		if (null != parseKeyMethod && null != parseValueMethod) {
			for (Map.Entry<String, String> toTransformEntry : original.entrySet()) {
				try {
					...
					throw new PreferenceException("Error tranforming Map<java.lang.String, java.lang.String> to Map<" + keyClass.getName() + ", " + valueClass.getName() + ">.", e);
				}
			}
