...
import info.novatec.novaspy.rcp.repository.RepositoryChangeListener;
...
public class ServerView extends ViewPart implements RepositoryChangeListener {
	 ...
	 * The ID of this view.
	 */
	public static final String ID = "novaspy.serverview";
	...
	private void updateServerItem(PShelfItem item, RepositoryDefinition repositoryDefinition) {
		if (repositoryDefinition.getServerStatusService().isOnline()) {
			item.setImage(NovaSpy.getDefault().getImage(NovaSpyConstants.IMG_SERVER_ONLINE));
			...
			item.setImage(NovaSpy.getDefault().getImage(NovaSpyConstants.IMG_SERVER_OFFLINE));
	...
	private class TreeViewDoubleClickListener implements IDoubleClickListener {
		...
		public void doubleClick(DoubleClickEvent event) {
			...
			try {
				handlerService.executeCommand("info.novatec.novaspy.rcp.commands.openView", null);
			} catch (Exception ex) {
