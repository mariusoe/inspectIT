...
import info.novatec.novaspy.cmr.service.IGlobalDataAccessService;
...
public class GlobalDataAccessService implements IGlobalDataAccessService {
	...
	public List<PlatformIdent> getConnectedAgents() {
		try {
			...
			NovaSpy.getDefault().createErrorDialog("There was an error retrieving the agent details from the CMR!", e, -1);
	...
	public List<? extends DefaultData> getLastDataObjects(DefaultData template, long timeInterval) {
		try {
			...
			NovaSpy.getDefault().createErrorDialog("There was an error retrieving the last data objects (time) from the CMR!", e, -1);
			return Collections.EMPTY_LIST;
		}
	}
	...
	public List<? extends DefaultData> getDataObjectsFromToDate(DefaultData template, Date fromDate, Date toDate) {
		try {
			...
			NovaSpy.getDefault().createErrorDialog("There was an error retrieving data objects from->to from the CMR!", e, -1);
			return Collections.EMPTY_LIST;
		}
	}
	...
	public DefaultData getLastDataObject(DefaultData template) {
		try {
			...
			NovaSpy.getDefault().createErrorDialog("There was an error retrieving the last data object from the CMR!", e, -1);
			return null;
		}
	}
	...
	public List<? extends DefaultData> getDataObjectsSinceId(DefaultData template) {
		try {
			...
			NovaSpy.getDefault().createErrorDialog("There was an error retrieving the data objects since id from the CMR!", e, -1);
			return Collections.EMPTY_LIST;
		}
	}
	...
	public List<? extends DefaultData> getDataObjectsSinceIdIgnoreMethodId(DefaultData template) {
		try {
			...
			NovaSpy.getDefault().createErrorDialog("There was an error retrieving the data objects since id (ignore method id) from the CMR!", e, -1);
			return Collections.EMPTY_LIST;
		}
	}
