...
public abstract class AbstractComparingValidator<T> implements IGroupedProperyValidator, ISinglePropertyValidator<T> {
	...
	public void validate(SingleProperty<? extends T> property, PropertyValidation propertyValidation) {
		...
		if (null == against) {
			ValidationError validationError = new ValidationError("Validation of  property " + property.getName() + " failed because literal (" + than + ") to compare against can not be parsed.");
	...
	public void validate(GroupedProperty groupProperty, PropertyValidation propertyValidation) {
		if (StringUtils.isEmpty(property)) {
			ValidationError validationError = new ValidationError("Validation of grouped property " + groupProperty.getName() + " failed because property logical name is not set.");
			propertyValidation.addValidationError(validationError);
			return;
		}

		if (StringUtils.isEmpty(than)) {
			ValidationError validationError = new ValidationError("Validation of grouped property " + groupProperty.getName()
		...
		if (null == compare) {
			ValidationError validationError = new ValidationError("Validation of grouped property " + groupProperty.getName() + " failed because property with logical name '" + property
					+ "' does not exist.");
			propertyValidation.addValidationError(validationError);
			return;
		}
		...
		if (null == against) {
			ValidationError validationError = new ValidationError("Validation of grouped property " + groupProperty.getName() + " failed because property with logical name '" + than
					+ "' does not exist.");
			propertyValidation.addValidationError(validationError);
			return;
		}
