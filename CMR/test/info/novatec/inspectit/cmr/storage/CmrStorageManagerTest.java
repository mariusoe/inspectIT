...
public class CmrStorageManagerTest extends AbstractTestNGLogSupport {
	...
	public void copyBufferToStorage() throws IOException, SerializationException, StorageException {
		...
		storageManager = spy(storageManager);
		when(storageDataDao.getAllDefaultDataForAgent(platformId)).thenReturn(data);

		// first with no auto-finalize
		storageManager.copyBufferToStorage(storageData, platformIdents, processors, false);
		verify(storageDataDao, times(1)).getAllDefaultDataForAgent(platformId);
		...
		verify(storageDataDao, times(2)).getAllDefaultDataForAgent(platformId);
